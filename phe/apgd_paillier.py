...
for exp in experiments:
    ...
    for E in Es:
        ...
        for i in range(10):  # repeat each experiment 100 times
            ...
            while True:
                ...
                y_dec = x0_dec_new + (x0_dec_new - x0_dec) * (k-1)/(k+2)  # we cannot perform this over palintext... (see paper)
                # the client encrypts the result and sends it to the cloud
                y_new = encrypt_vector(pubkey, fp_vector(y_dec))
                ...
                print(k, 'OPT:', f'{objective((x0_dec)):.3f}')
                ...
                if np.allclose(x0_dec, x0_dec_new):  # convergence
                    ...
                    results = np.vstack((results, np.asarray([n, e, correct_value_after, k + 1, time.time() - start_time, 1, 1, (objective(x0_dec_new) - objective(sol["x"]))])))
                    break
                else:
                    x0_dec = x0_dec_new
                    y = y_new
